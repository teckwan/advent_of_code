...
fn find_gear(x: usize, y: usize, map: &Vec<Vec<char>>) -> Option<(usize, usize)> {
    let min_y = if y == 0 { y } else { y - 1 };
    let max_y = if y == map.len() - 1 { y } else { y + 1 };
...
fn part_2(input: &str) -> u32 {
    let map =
        Vec::from_iter(input.lines().map(|line| Vec::from_iter(line.chars())));
    let mut gear_map = Vec::from_iter(map.iter().map(|line| vec![Vec::new(); line.len()]));
        ...
        for x in 0..axis.len() {
            if map[y][x].is_numeric() {
                ...
                match find_gear(x, y, &map) {
                    Some(c) => {
                        if !gears.contains(&c) { gears.push(c) };
                    },
                    None => {}
                }
            } else if number != 0 {
